...
function calcStatus(dt) {
  ...
  return Math.ceil((v-new Date())/864e5) < 0 ? "vencida" : Math.ceil((v-new Date())/864e5)<=30 ? "avencer" : "vigente";
}
...
const DADOS = [
  {id:1,material:"Porca Sextavada 8mm Inox 304",codigo:"MAT-001",categoria:"Fixação",unidade:"UNID.",contrato:"Contrato 012/2022",dataBase:"Nov/2024",dataElaboracao:"2024-11-15",bdi:17.32,desconto:2.0,quantidade:100,observacoes:"Inox passivado 304",imagem:null,fornecedores:[{nome:"Parafuso Fácil",url:"",valor:0.85},{nome:"Jofepar",url:"",valor:0.82},{nome:"Lojas Mixpar",url:"",valor:0.91}]},
  {id:2,material:"Válvula Termostática Danfoss R22 – 12TR",codigo:"MAT-010",categoria:"Refrigeração",unidade:"UNID.",contrato:"Contrato 012/2022",dataBase:"Nov/2024",dataElaboracao:"2024-11-15",bdi:17.32,desconto:2.0,quantidade:1,observacoes:"Modelo 067N2009",imagem:null,fornecedores:[{nome:"Chiller Peças",url:"",valor:1031.11},{nome:"Jet Frio",url:"",valor:1008.99},{nome:"Cibrel",url:"",valor:1375.53}]},
  {id:3,material:"Graxa Azul FAG 500g",codigo:"MAT-005",categoria:"Lubrificação",unidade:"UNID.",contrato:"Contrato 012/2022",dataBase:"Nov/2024",dataElaboracao:"2024-02-10",bdi:17.32,desconto:2.0,quantidade:1,observacoes:"Para rolamentos",imagem:null,fornecedores:[{nome:"C3 Multimarcas",url:"",valor:81.18},{nome:"Loja Proelis",url:"",valor:85.49},{nome:"Disk Peças",url:"",valor:90.45}]},
  {id:4,material:"Tubo de Cobre Classe A 42mm 1.1/2",codigo:"MAT-006",categoria:"Tubulação",unidade:"M",contrato:"Contrato 012/2022",dataBase:"Nov/2024",dataElaboracao:"2024-11-15",bdi:17.32,desconto:2.0,quantidade:1,observacoes:"Referência SINAPI 39751",imagem:null,fornecedores:[{nome:"SINAPI 39751",url:"",valor:179.06}]},
  {id:5,material:"Placa Universal Split Hi-Wall Suryha",codigo:"MAT-008",categoria:"Elétrico",unidade:"UNID.",contrato:"Contrato 012/2022",dataBase:"Nov/2024",dataElaboracao:"2024-11-15",bdi:17.32,desconto:2.0,quantidade:3,observacoes:"80150 Suryha",imagem:null,fornecedores:[{nome:"Sardanha Refrigeração",url:"",valor:164.53},{nome:"Refritron",url:"",valor:152.63},{nome:"Webinstalar",url:"",valor:157.39}]},
];
  ...
  return (
    <div style={{background:"#f8fafc",border:"1px solid #e2e8f0",borderRadius:10,padding:"12px 14px",marginTop:10}}>
      ...
      <table style={{width:"100%",borderCollapse:"collapse",fontSize:12}}>
        <thead><tr style={{background:"#f1f5f9"}}>{["Fonte","Valor","Variação s/ média geral","Status"].map(h=><th key={h} style={{padding:"6px 10px",textAlign:"left",fontSize:11,fontWeight:600,color:"#64748b",borderBottom:"1px solid #e2e8f0"}}>{h}</th>)}</tr></thead>
    ...
    <div style={{position:"fixed",inset:0,background:"rgba(0,0,0,.45)",zIndex:200,display:"flex",alignItems:"center",justifyContent:"center",padding:16}} onClick={onClose}>
      <div onClick={ev=>ev.stopPropagation()} style={{background:"#fff",borderRadius:16,width:"100%",maxWidth:700,maxHeight:"94vh",overflowY:"auto",padding:26}}>
        ...
        <div style={{display:"grid",gridTemplateColumns:"1fr 1fr 1fr",gap:10,marginBottom:20}}>
          <div style={{background:"#f8fafc",border:"1px solid #e2e8f0",borderRadius:10,padding:"12px 14px"}}><div style={{fontSize:10,color:"#94a3b8",fontWeight:600,marginBottom:4}}>MÉDIA SANEADA — base</div><div style={{fontSize:17,fontWeight:800,color:"#0f172a"}}>R$ {brl(calc.mediaSaneada)}</div><div style={{fontSize:10,color:"#94a3b8",marginTop:3}}>base para os cálculos</div></div>
          <div style={{background:"#dbeafe",border:"1px solid #93c5fd",borderRadius:10,padding:"12px 14px"}}><div style={{fontSize:10,color:"#1e40af",fontWeight:600,marginBottom:4}}>VALOR C/ BDI ({form.bdi||0}%)</div><div style={{fontSize:17,fontWeight:800,color:"#1d4ed8"}}>R$ {brl(calc.comBDI)}</div><div style={{fontSize:10,color:"#3b82f6",marginTop:3}}>média × (1 + BDI%)</div></div>
          <div style={{background:"#dcfce7",border:"1px solid #86efac",borderRadius:10,padding:"12px 14px"}}><div style={{fontSize:10,color:"#166534",fontWeight:600,marginBottom:4}}>VALOR FINAL ({form.desconto||0}% desc.)</div><div style={{fontSize:17,fontWeight:800,color:"#16a34a"}}>R$ {brl(calc.final)}</div><div style={{fontSize:10,color:"#22c55e",marginTop:3}}>c/BDI × (1 − desc.%)</div></div>
        </div>
...
function TelaLogin({onLogin}) {
  ...
  async function handleLogin() {
    ...
    try {
      await signInWithEmailAndPassword(auth, loginToEmail(login), senha);
    } catch(e) {
  ...
  return (
    <div style={{minHeight:"100vh",background:"#f8fafc",display:"flex",alignItems:"center",justifyContent:"center",fontFamily:"'DM Sans','Segoe UI',sans-serif"}}>
      <div style={{background:"#fff",borderRadius:18,border:"1px solid #e2e8f0",padding:"36px 40px",width:"100%",maxWidth:380,boxShadow:"0 8px 32px rgba(0,0,0,.07)"}}>
        ...
        <button onClick={handleLogin} disabled={loading} style={{width:"100%",background:"#0f172a",color:"#fff",border:"none",borderRadius:9,padding:"11px",fontSize:14,fontWeight:700,cursor:loading?"not-allowed":"pointer",opacity:loading?.7:1}}>
...
export default function App() {
  const [usuario,setUsuario]=useState(null);   // perfil do Firestore
  const [authUser,setAuthUser]=useState(null); // objeto Firebase Auth
    ...
    const unsub = onAuthStateChanged(auth, async (fireUser) => {
      if (fireUser) {
        ...
        if (snap.exists()) {
          ...
          // Primeiro admin: cria perfil automaticamente
          const login = fireUser.email.replace("@cotafrio.app","");
          const perfil = {uid:fireUser.uid,login,nome:login,papel:"admin",criadoEm:new Date().toISOString()};
  ...
  async function excluirCot(id) {
    ...
    catch(e){showToast("Erro ao excluir cotação.","erro");}
  }
      ...
      <div style={{background:"#fff",borderBottom:"1px solid #e2e8f0",padding:"0 24px",display:"flex",alignItems:"center",height:58,gap:14,position:"sticky",top:0,zIndex:100}}>
        ...
        <div style={{display:"flex",alignItems:"center",gap:8,borderLeft:"1px solid #e2e8f0",paddingLeft:14}}>
          <div style={{textAlign:"right"}}>
            <div style={{fontSize:12,fontWeight:600,color:"#0f172a"}}>{usuario?.nome||usuario?.login}</div>

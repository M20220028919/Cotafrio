...
function enriquecer(c) {
  if (c.precoDireto) {
    return { ...c, fornecedores: [], mediaGeral: c.valorFinalDireto, mediaSaneada: c.valorFinalDireto, menorSaneado: c.valorFinalDireto, precoFinalBDI: c.valorFinalDireto, precoFinalDesconto: c.valorFinalDireto, status: calcStatusCot(c.dataElaboracao) };
  }
...
const emptyFormContrato = {
  ...
  bdi: 17.32, desconto: 2.0, materiais: [],
  fiscal:"",observacoes:""
};
    ...
    <div style={{position:"fixed",inset:0,background:"rgba(0,0,0,.45)",zIndex:200,display:"flex",alignItems:"center",justifyContent:"center",padding:16}} onClick={onClose}>
      <div onClick={ev=>ev.stopPropagation()} style={{background:"#fff",borderRadius:16,width:"100%",maxWidth:700,maxHeight:"94vh",overflowY:"auto",padding:26}}>
        ...
        <div style={{fontSize:11,fontWeight:600,color:"#94a3b8",letterSpacing:.6,marginBottom:10, display:"flex", justifyContent:"space-between", alignItems:"center"}}>
            ...
            <label style={{display: "flex", alignItems: "center", gap: 6, fontSize: 12, cursor: "pointer", color: "#0f172a"}}>
        ...
        {form.precoDireto ? (
            <div style={{background:"#f8fafc",borderRadius:10,padding:"14px",marginBottom:20}}>
                {lbl("Valor Final (R$) - Unidade")}
                <input type="number" step="0.01" min="0" placeholder="0,00" value={form.valorFinalDireto} onChange={ev=>setForm(p=>({...p,valorFinalDireto:parseFloat(ev.target.value)||0}))} style={{border:"1px solid #e2e8f0",borderRadius:7,padding:"7px 10px",fontSize:12, width: "200px"}}/>
                <div style={{fontSize: 11, color: "#64748b", marginTop: 8}}>O valor inserido será considerado como preço final já com BDI e descontos aplicados.</div>
            </div>
        ) : (
            <>
                ...
                <div style={{fontSize:11,fontWeight:600,color:"#94a3b8",letterSpacing:.6,margin:"20px 0 10px"}}>PARÂMETROS DE CÁLCULO (Automáticos do Contrato)</div>
                <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:12,marginBottom:12}}>
                    <div style={{background:"#f1f5f9", padding: "8px 12px", borderRadius: 8, fontSize: 13, color: "#64748b"}}>BDI: {form.bdi||0}%</div>
                    <div style={{background:"#f1f5f9", padding: "8px 12px", borderRadius: 8, fontSize: 13, color: "#64748b"}}>Desconto: {form.desconto||0}%</div>
                </div>
        ...
        <div style={{position:"fixed",inset:0,background:"rgba(0,0,0,.45)",zIndex:200,display:"flex",alignItems:"center",justifyContent:"center",padding:20}} onClick={()=>setDetalhe(null)}>
          <div onClick={ev=>ev.stopPropagation()} style={{background:"#fff",borderRadius:16,width:"100%",maxWidth:640,maxHeight:"92vh",overflowY:"auto",padding:28}}>
            ...
            {[
              ...
              {titulo:"VALORES E PARÂMETROS",campos:[{l:"Valor total",v:"R$ "+brl(detalhe.valorTotal)},{l:"BDI (%)",v:detalhe.bdi},{l:"Desconto (%)",v:detalhe.desconto}]},
            ].map(sec=>(
                  ...
                  <div key={cid} style={{marginBottom: 32}}>
                      <div style={{borderBottom: "2px solid #e2e8f0", paddingBottom: 8, marginBottom: 16, display: "flex", alignItems: "baseline", gap: 10}}>
                          <h3 style={{margin: 0, fontSize: 16, color: "#0f172a"}}>
                              {ctr ? `${ctr.numero} — ${ctr.contratadaRazaoSocial || 'Sem Nome'}` : 'Cotações Sem Contrato (Antigas)'}
                          </h3>
                          <span style={{fontSize: 12, color: "#64748b"}}>{lista.length} iten(s)</span>
                      </div>
                          ...
                          {lista.map(c=>{
                            ...
                            return(
                              <div key={c.id} onClick={()=>setDetalhe(c)} style={{background:"#fff",border:"1px solid #e2e8f0",borderRadius:14,overflow:"hidden",cursor:"pointer", position: "relative"}}
                                ...
                                {c.precoDireto && <div style={{position:"absolute", top: 10, left: -25, background:"#b45309", color:"#fff", fontSize: 9, fontWeight:"bold", padding:"2px 30px", transform:"rotate(-45deg)"}}>MANUAL</div>}
